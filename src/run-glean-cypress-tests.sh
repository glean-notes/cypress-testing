...
GLEAN_URL=https://cypress-testing.dev.glean.ninja
ADMIN_URL=https://cypress-testing.dev.glean.ninja/admin
COOKIE_DOMAIN=cypress-testing.dev.glean.ninja

CYPRESS_baseUrl=$GLEAN_URL cypress run --browser="chrome" --headed --record --key unused --project . --env COOKIE_DOMAIN="$COOKIE_DOMAIN",TEST_HELPER_USR="$TEST_HELPER_USR",TEST_HELPER_PSW="$TEST_HELPER_PSW",ADMIN_URL="$ADMIN_URL",GLEAN_URL="$GLEAN_URL",LOG_FAILURES_TO_KIBANA=$LOG_FAILURES_TO_KIBANA,DELETE_SUCCESS_VIDEOS=true --spec "./integration/ci/**/*.spec.ts"
